export type TabType = 'over_limite' | 'over_frente' | 'over_longa' | 'all';
...
export interface OverMarketTarget {
  tab: TabType;
  targetLine: number; // e.g. 1.5, 2.5, 3.5
  referenceOdd: number; // e.g. 2.00 or 3.00
  foundOdd: number | null; // e.g. 2.18 or null if N/D
...
export interface SystemSettings {
  pollIntervalSeconds: number; // 30, 15, 10, 5
  isAutoRefreshActive: boolean;
  overLimiteRefOdd: number; // 2.00
  overFrenteRefOdd: number; // 3.00
  overLongaRefOdd: number; // 3.00
